...
<section id="farm_hope" style="margin: 0 auto; float: inherit">
    <?php if (!$corders_info && !$corders_info2 && !$corders_info3 && !$corders_info4) { ?>
        <p>なし。</p>
    <?php } else { ?>
        <br>
        <h2 style="text-align: center">注文現況</h2>
    <div><span style='color: red;' onclick="displayon('cl3')">完了 : <?php if(isset($status_info[0])){ echo $status_info[0]->count; } else { echo "0"; } ?> </span> / <a style='color: limegreen' onclick="cordering()"> 取引中 : <?php if(isset($status_info2[0])) echo $status_info2[0]->count; else echo "0";  ?></a> / <a>中止 : <?php if(isset($status_info3[0])) echo $status_info3[0]->count; else echo "0";  ?> </a></div>
    <br>
    <table id="table">
        ...
        <?php foreach($corders_info2 as $row) { ?>
            <tr align="center" class="cl1">
                <td style="background-color:<?php if($row->costatus == 0 || $row->costatus == 2) { echo "#F6FFF2";} else if($row->costatus == 1 || $row->costatus == 3) {echo "#F1FFFF";} else if($row->costatus == 4) {echo "#FFF2F2";} else {echo "#F5F5F5";} ?>">
                    <img class="timg" src="/public/assets/img/crops/<?= $row->cpname ?>" onclick="location.href='/Farm/farmHopeDetail/<?= $row->cono ?>'">
                    <br><?= $row->fname ?> - <?= $row->cname ?></td>
                <td style="padding: 10px 15px;"><?= $row->costock ?> kg</td>
                <td style="padding: 0 12px;"><div onclick="location.href='/Farm/farmHopeDetail/<?= $row->cono ?>'"><?php
                        if($row->costatus == 0) {
                            echo "<span style='color: limegreen; cursor: pointer'>取引を確認してください</span>";
                        } else if($row->costatus == 1) {
                            echo "<span style='color: deepskyblue; cursor: pointer'>入金を待っています</span>";
                        } else if($row->costatus == 2) {
                            echo "<span style='color: limegreen; cursor: pointer'>発送してください</span>";
                        } else if($row->costatus == 3) {
                            echo "<span style='color: deepskyblue; cursor: pointer'>発送中</span>";
                        } else if($row->costatus == 4) {
                            echo "<span style='color: red; cursor: pointer'>終了</span>";
                        } else {
                            echo "<span style='color: black; cursor: pointer'>取引を中止しました</span>";
                        }
                        ?></div></td>
